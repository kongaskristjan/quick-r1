...
def log_completion(completions: list[str], target: list[int], numss: list[list[int]], **kwargs) -> list[float]:
    think, answer = get_think_and_answer(completions[0])
    format_correct = format_reward(completions[:1])[0] > 0
    expression_format_correct = expression_format_reward(completions[:1], numss[:1])[0] > 0
    equation_correct = equation_reward(completions[:1], target[:1], numss[:1])[0] > 0
